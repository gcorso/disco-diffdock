...
from datasets_utils.process_mols import read_molecule, get_rec_graph, generate_conformer, \
...
class NoiseTransform(BaseTransform):
    def __init__(self, t_to_sigma, no_torsion, all_atom, alpha, beta):
...
class PDBBind(Dataset):
    def __init__(self, root, transform=None, cache_path='data/cache', split_path='data/', limit_complexes=0,
                 ...
                 multiplicity=1, require_rdkit=False):
        ...
        if (not os.path.exists(os.path.join(self.full_cache_path, "heterographs.pkl"))) or require_rdkit:
            self.full_cache_path += '_withrdkit'
    ...
    def get(self, idx):
        ...
        complex_graph['ligand'].orig_pos_torch = torch.from_numpy(
            complex_graph['ligand'].orig_pos - complex_graph.original_center.cpu().numpy()).float()
...
def construct_loader(args, t_to_sigma):
    ...
    train_dataset = PDBBind(cache_path=args.cache_path, split_path=args.split_train, keep_original=True,
                            num_conformers=args.num_conformers, multiplicity=args.train_multiplicity, **common_args)
