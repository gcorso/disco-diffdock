...
def train_epoch(model, loader, optimizer, device, t_to_sigma, loss_fn, ema_weigths, use_latent, training_latent_temperature=1.0):
    model.train()
    ...
    for data in tqdm(loader, total=len(loader)):
        ...
        while runbatch:
            ...
            try:
                ...
                loss.backward()
                optimizer.step()
                ema_weigths.update(model.module.score_model.parameters() if device.type == 'cuda' else model.score_model.parameters())
                meter.add([loss.cpu().detach(), tr_loss, rot_loss, tor_loss, tr_base_loss, rot_base_loss, tor_base_loss])
            except RuntimeError as e:
                if 'out of memory' in str(e):
                    print('| WARNING: ran out of memory, trying to reduce batch size')
                    for p in model.parameters():
                        if p.grad is not None:
                            del p.grad  # free some memory
                    torch.cuda.empty_cache()
                    runbatch = True
                    data = data[:int(len(data) * 0.8)] # reduce batch size
...
def test_epoch(model, loader, device, t_to_sigma, loss_fn, test_sigma_intervals=False, use_latent=False, training_latent_temperature=1.0):
    model.eval()
    meter = AverageMeter(['loss', 'tr_loss', 'rot_loss', 'tor_loss', 'tr_base_loss', 'rot_base_loss', 'tor_base_loss'],
...
def inference_epoch(model, complex_graphs, device, t_to_sigma, args, use_latent=False):
    ...
    for orig_complex_graph in tqdm(loader):
        ...
        while predictions_list == None:
            try:
                predictions_list, confidences = sampling(data_list=data_list, model=model if device.type=='cuda' else model,
                                                         inference_steps=args.inference_steps,
                                                         tr_schedule=tr_schedule, rot_schedule=rot_schedule,
                                                         tor_schedule=tor_schedule, use_latent=use_latent,
                                                         device=device, t_to_sigma=t_to_sigma, model_args=args,
                                                         gumbel_latent_temperature=args.sampling_latent_temperature)
